langInfo = {
    "engName": "Chinese (Simplified)",
    "orgiName": "简体中文",
    "code": "zh_CN",
    "font": [{"windows": "Microsoft YaHei", "macos": "PingFang SC", "linux": "Noto Sans SC"}],
    "machineTranslated": False,
    "useKKGithub": True,
    "rtl": False
}

infomation = {
    "wslWarning": "发现您正在使用 WSL, 实际上, 我们更推荐您直接使用 Windows 版本以避免潜在的 BUG",
    "cygwinWarning": "发现您正在使用 Cygwin, 实际上, 我们更推荐您直接使用 Windows 版本以避免潜在的 BUG",
    "nonTestWarning": "除 Windows 外, 该软件没有做过稳定性测试, 可能会存在一些开发者也不清楚的 BUG 需要修复",
    "machineTranslate": "您所使用的语言是机器翻译得到的，可能会存在一些语言理解性问题",
    "updateUseKKGithubMirror": "注: 使用该语言时检查更新将切换为 KKGitHub 镜像站"
}

menuBar = {
    "files": "文件",
    "openFile": "打开文件",
    "openSonglist": "导入播放列表",
    "getFromMusicWebsite": "从音乐网站获取",
    "getWebMusic": "从网易云获取音乐",
    "getWebAlbum": "获取专辑/歌单",
    "exit": "退出",
    "play": "播放",
    "media": "媒体",
    "channels": "左右声道",
    "balance": "平衡",
    "shiftLeft": "向左移",
    "shiftRight": "向右移",
    "position": "进度",
    "forward10s": "快进 10s",
    "back10s": "后退 10s",
    "speed": "倍速",
    "0.5x": "0.5x",
    "1x": "1x",
    "1.5x": "1.5x",
    "2x": "2x",
    "volume": "音量",
    "lyrics": "歌词",
    "mode": "模式",
    "playInOrder": "顺序播放",
    "loop": "列表循环",
    "repeat": "单曲循环",
    "shuffle": "随机播放",
    "audioInfo": "媒体信息",
    "help": "帮助",
    "update": "检查更新",
    "about": "关于"
}

mainMenu = {
    "none": "无",
    "unknownMusic": "未知歌曲",
    "unknownArtist": "未知作曲家",
    "unknownAlbum": "未知专辑",
    "moreInfo": "详细信息",
    "songLoaded": "已加载歌曲:",
    "about": "关于",
    "beenTop": "已置顶",
    "beenUntop": "已取消置顶",
    "resetMenuBar": "按下 Ctrl+H 复原顶栏!",
    "enableLoop": "已启用循环播放",
    "disableLoop": "已禁用循环播放"
}

tooltips = {
    "alwaysOnTop": "置顶",
    "cancelAlwaysOnTop": "取消置顶",
    "hideMenuBar": "隐藏顶栏 (Ctrl+H)",
    "audioPosition": "歌曲进度",
    "skipPrevious": "上一首",
    "playOrPause": "播放/暂停",
    "skipNext": "下一首",
    "playInLoop": "循环播放",
    "playRepeat": "单曲循环",
    "volume": "音量",
    "lyrics": "歌词",
    "songList": "播放列表",
    "audioInfo": "媒体信息",
    "settings": "设置"
}

songList = {
    "songList": "播放列表"
}

dialog = {
    "songIdInput": "输入歌曲 ID:",
    "songIdHint": "(目前仅限网易云 + 非会员歌曲)",
    "vipOrNoCopyrightAlert": "抱歉, 不支持获取会员或由于版权问题无法播放的歌曲",
    "errorPrompt": "请输入正确的歌曲 ID",
    "updateTimeout": "检查更新超时, 请检查网络连接",
    "youAreUsingLatest": "你正在使用最新版本",
    "findUpdate": "发现更新",
    "update": "更新",
    "cancel": "取消",
    "ok": "确定",
    "githubRepo": "GitHub 仓库"
}

update = {
    "attention": "\n \r## 注意\n \r 本版本为预发布版本, 可能存在许多 BUG!",
    "darwinCompatibility": "\n \r## 兼容性须知\n \r 您似乎在使用 macOS, \n \r我们使用了 Github Actions 编译适用于 macOS 的版本, 由于没有测试设备, 可能存在稳定性问题",
    "linuxCompatibility": "\n \r## 兼容性须知\n \r 您似乎在使用 Linux, \n \r我们使用了 Github Actions 编译适用于 Linux 的版本, 由于没有进行过测试, 可能存在稳定性问题",
    "contentCurrentVer": "- 当前版本: ",
    "contentLatestVer": "\n \r- 新版本: ",
    "contentMore": "\n \r## 详细信息\n \r"
}

sets = {
    "settings": "设置",
    "note": "这里也可以用空格键来控制音乐的播放与暂停哟~\n大部分设置将在重启后生效",
    "pleaseSelect": "请选择...",
    "construction": "正在施工",
    "language": "语言",
    "systemDefault": "系统默认",
    "langNameEng": "英文名: ",
    "langNameOrigial": "名称: ",
    "langCode": "语言代码: ",
    "langFont": "语言字体: ",
    "langRTL": "默认 RTL 模式: ",
    "appearances": "外观",
    "colorMode": "亮暗模式: ",
    "colorLight": "亮色",
    "colorDark": "暗色",
    "colorSchemes": "配色方案: ",
    "blue": "蓝色",
    "pink": "粉色",
    "green": "绿色",
    "brown": "棕色",
    "purple": "紫色",
    "appearancesSys": "外观使用当前语言中设置",
    "rtl": "从右到左模式",
    "play": "播放",
    "immediatelyPlay": "加载歌曲后立即播放",
    "defaultLoop": "默认循环播放",
    "defaultVolume": "默认音量: ",
    "lyrics": "歌词",
    "lyricsDefaultVis": "歌词默认显示",
    "online": "在线",
    "onlineMusic": "在线歌曲: ",
    "inputAPI": "输入 API 地址: ",
    "setToDefault": "恢复默认",
    "webAPIInfo": "必须使用 https://github.com/Kevin0z0/Python_NetEaseMusicAPI 的部署方案以保证兼容性!",
    "webUpdate": "更新: ",
    "onlineSys": "在线更新 API 使用当前语言中设置",
    "systemIntegration": "系统集成",
    "enableSMTC": "启用 SMTC (未完成, 仅限 Windows)",
    "enableToastNotify": "启用吐司通知 (仅限 Windows)",
    "advance": "高级",
    "delConfigAndClose": "删除配置并关闭软件",
    "delLogFile": "删除日志文件",
    "feedback": "反馈",
    "bugReport": "报告 BUG",
    "shareIdeas": "分享新功能想法",
    "discussion": "加入讨论"
}