...
async def main(page):
    ...
    def readSong(audioPathTemp):
        ...
        audioPathTemp = None
        audioInfoUpdate()
        page.title = work.audioArtistText + " - " + work.audioTitleText + " | Simplay Player"
    ...
    audioProgressBar = ft.Slider(
        min=0,
        max=1000,
        tooltip=lang.tooltips["audioPosition"],
        on_change_start=autoStopKeepAudioProgress,
    ...
    volume_silder = ft.Slider(
        min=0,
        max=100,
        divisions=100,
        label="{value}",
        value=cfg.cfgData["play"][0]["defaultVolume"],
        scale=0.9,
        opacity=0.9,
        on_change=volumeChange,
    )
    volume_panel = ft.Card(content=volume_silder, visible=False, height=46, width=200, shape=ft.RoundedRectangleBorder(radius=100))

    lyrics_btn = ft.IconButton(icon=ft.Icons.LYRICS, tooltip=lang.tooltips["lyrics"], icon_size=20, visible=True, on_click=lyricShow)
    playInRepeat_btn = ft.IconButton(icon=ft.Icons.REPEAT_ONE_OUTLINED, tooltip=lang.tooltips["playRepeat"], icon_size=20, visible=True, on_click=enableOrDisableRepeat)
    audioList_btn = ft.IconButton(icon=ft.Icons.LIBRARY_MUSIC_OUTLINED, tooltip=lang.tooltips["songList"], icon_size=20, on_click=audioListCtrl)

    songlist_tiles = ft.Column(controls=[], height=380, spacing=0, scroll=ft.ScrollMode.AUTO)
    audioList_menu = ft.Container(
        content=ft.Column(
            controls=[
                ft.Row(
                    controls=[ft.Text(value=lang.songList["songList"], size=20, weight=ft.FontWeight.BOLD),
    ...
    audioInfo_btn = ft.IconButton(icon=ft.Icons.INFO_OUTLINE, tooltip=lang.tooltips["audioInfo"], icon_size=20, on_click=openAudioInfoDlg)
    settings_btn = ft.IconButton(icon=ft.Icons.SETTINGS_OUTLINED, tooltip=lang.tooltips["settings"], icon_size=20, on_click=displaySettings)
    ...
    if "pre" in ver:
        releaseWarning.value = f"Pre-release version for testing purposes only! Current Version: {ver}"
        log_init.logging.warning("You are using a pre-release version")
        releaseWarning.visible = True
        page.update()
    if "debug" in ver:
        releaseWarning.value = f"Debug version for development use! Current Version: {ver}"
        log_init.logging.warning("You are using a debug version")
        releaseWarning.visible = True
        page.update()
    if "experiment" in ver:
        releaseWarning.value = f"Experimental version for develop and testing! Current Version: {ver}"
